  ...
  return [
    {
      ...
      timeline: '5+ years',
      description: 'Systematic Investment Plan in diversified equity mutual funds',
      examples: ['Axis Bluechip Fund', 'ICICI Pru Bluechip Fund', 'SBI Large & Midcap Fund'],
    ...
    },
    {
      ...
      expectedReturn: '8-12%',
      timeline: '8 years',
      description: 'Government-issued bonds backed by gold with additional interest',
...
export function getProfessionalInvestmentOptions(): InvestmentOption[] {
  return [
    ...
    },
    {
      ...
      pros: ['High return potential', 'Portfolio diversification', 'Startup exposure'],
    ...
    },
    {
      ...
      pros: ['Global diversification', 'Currency hedge', 'Access to global giants'],
...
export function getRecommendationsBasedOnProfile(
  ...
  const options = userType === 'personal' ? getPersonalInvestmentOptions() : getProfessionalInvestmentOptions();
  const riskMap = { conservative: 'Low', moderate: 'Medium', aggressive: 'High' };
  const userRisk = riskMap[riskTolerance];
  
  return options
    .filter(option => {
      if (userRisk === 'Low') return ['Low', 'Medium'].includes(option.riskLevel);
      if (userRisk === 'Medium') return ['Low', 'Medium', 'Medium-High'].includes(option.riskLevel);
      return true; // High risk tolerance accepts all
    })
    .map(option => ({
      ...option,
      suggestedAllocation: Math.round((monthlyIncome * option.allocation[riskTolerance]) / 100),
      suitabilityScore: calculateSuitabilityScore(option, riskTolerance, age, monthlyIncome)
    }))
...
function calculateSuitabilityScore(
  ...
  score += riskAlignment[riskTolerance][option.riskLevel as keyof typeof riskAlignment.conservative] || 0;
  
  return Math.max(0, Math.min(100, score));
}

export function getInvestmentDetails(investmentId: string): InvestmentOption | undefined {
  const allOptions = [...getPersonalInvestmentOptions(), ...getProfessionalInvestmentOptions()];
